...
(defun present-group-members (ch)
  (cond
    ((or (null ch)
         (not (aff-flagged ch +aff-group+)))
     nil)
    (t
     (list* (or (master-of ch) ch)
            (remove-if-not (lambda (f)
                             (and (aff-flagged f +aff-group+)
                                  (eql (in-room-of f)
                                       (in-room-of ch))))
                           (followers-of (or (master-of ch) ch)))))))

(eval-when (:load-toplevel :compile-toplevel :execute)
  (defun build-ability-macro (kind name flags body)
         ...
         (defun ,func-name (caster target level saved)
           ...
           (let (,@(when (or (member 'affect body-syms)
             ...
             (macrolet ((duration (new-duration)
               ...
               (flet ((do-spell (target)
                        (unless (affected-by-spell target ,id-name)
                          ,@body)))
                 ,@(cond
                    ((member :masses flags)
                     `((dolist (target (people-of (in-room-of caster)))
                         (when (member caster (fighting-of target))
                           (do-spell target)))))
                    ((member :group flags)
                     `((when caster
                         (dolist (target (present-group-members caster))
                           (do-spell target)))))
                    ((member :area flags)
                     `((dolist (target (people-of (in-room-of caster)))
                         (unless (eql caster target)
                           (do-spell target)))))
                    (t
                     `((do-spell target))))))))
...
(defmacro define-spell (name (&rest flags) &body body)
  (build-ability-macro "SPELL" name flags body))

(defmacro define-song (name (&rest flags) &body body)
  (build-ability-macro "SONG" name flags body))
...
(define-song alrons-aria (:group)
  (duration (+ 3 (floor level 4)))
  (affect +apply-hitroll+ (dice 2 (+ 1 (floor level 8))))
  (affect +apply-saving-spell+ (- (dice 1 (+ 1 (floor level 8)))))
  (set-affbit 1 +aff-confidence+)
  (unaffect +spell-fear+)
  (to-target "Your confidence soars!"))

(define-song verse-of-valor ()
  (duration (+ 6 (floor level 8)))
  (affect +apply-hitroll+ (+ 5 (random-range 0 6) (floor level 25)))
  (to-target "The valor of heroes gone comes crashing into your mind!"))
