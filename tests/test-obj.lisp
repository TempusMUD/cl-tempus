...
(deftest obj-equip ()
  (with-fixtures ((alice mock-player)
                  (obj mock-object :name "a magic ring"))
...
(deftest affect-total ()
  (with-fixtures ((alice mock-player)
                  (equip mock-object :name "a magic ring")
                  (implant mock-object :name "an internal booster"))
    ;; Set up alice
...
(deftest get-command-ok ()
  (with-fixtures ((alice mock-player)
                  (bob mock-player)
                  (obj mock-object :name "some plate armor"))
    ...
    (char-output-has alice "You get some plate armor.~%")
    (char-output-has bob "Alice gets some plate armor.~%")))
...
(deftest get-all-dot-command ()
  (with-fixtures ((alice mock-player)
    ...
    (char-output-has alice "You get some plate armor. (x2)~%")
    (is (eql alice (tempus::carried-by-of armor-1)))
    (is (eql alice (tempus::carried-by-of armor-2)))))
...
(deftest get-command-no-take ()
  (with-fixtures ((alice mock-player)
    ...
    (char-output-has alice "Some plate armor: you can't take that!~%")))
...
(deftest get-gold ()
  (with-fixtures ((alice mock-player)
    ...
    (char-output-has alice "You get a pile of gold.~%There were 12345 coins.~%")))

(deftest get-cash ()
  (with-fixtures ((alice mock-player)
                  (obj mock-object :name "a pile of cash"))
    (setf (tempus::wear-flags-of obj) tempus::+item-wear-take+)
    (setf (tempus::kind-of obj) tempus::+item-money+)
    (setf (aref (tempus::value-of obj) 0) 12345)
    ...
    (char-output-has alice "You get a pile of cash.~%There were 12345 credits.~%")))
...
(deftest get-from-command ()
  (with-fixtures ((alice mock-player)
    ...
    (char-output-has alice "You get some plate armor from a treasure chest.~%")
    (char-output-has bob "Alice gets some plate armor from a treasure chest.~%")
    (is (eql alice (tempus::carried-by-of armor-1)))))
...
(deftest get-all-from-command ()
  (with-fixtures ((alice mock-player)
    ...
    (char-output-has alice "You get some plate armor from a treasure chest. (x2)~%")
    (char-output-has bob "Alice gets some plate armor from a treasure chest. (x2)~%")
    (is (eql alice (tempus::carried-by-of armor-1)))
    (is (eql alice (tempus::carried-by-of armor-2)))))
...
(deftest get-from-container/all-from-empty-container/failure ()
  (with-fixtures ((alice mock-player)
    ...
    (char-output-has alice "You didn't find anything to take in a treasure chest.~%")))
...
(deftest get-from-container/non-matching-objects/failure ()
  (with-fixtures ((alice mock-player)
    ...
    (char-output-has alice "You didn't find anything in a treasure chest that looks like a potion.~%")
    (clear-mock-buffers alice)
    (tempus::get-from-container alice "all.egg" chest)
    (char-output-has alice "You didn't find anything in a treasure chest that looks like an egg.~%")))
...
(deftest inventory-command ()
  (with-fixtures ((alice mock-player)
    ...
    (char-output-has alice "You are carrying:~%some plate armor~%")))
...
(deftest give-money-commands ()
  (with-fixtures ((alice mock-player)
    ...
    (char-output-has alice "You give 5000 coins to Bob.~%")
    (char-output-has bob "Alice gives 5000 coins to you.~%")
    (char-output-has eva "Alice gives some coins to Bob.~%")
    ...
    (char-output-has alice "You give 5000 credits to Bob.~%")
    (char-output-has bob "Alice gives 5000 credits to you.~%")
    (char-output-has eva "Alice gives some credits to Bob.~%")
...
(deftest drink-command ()
  (with-fixtures ((alice mock-player)
                  (bob mock-player)
                  (glass mock-object :name "a glass of water"))
    ...
    (char-output-has alice "Your thirst has been quenched.~%")
...
(deftest drink-command-poison ()
  (with-fixtures ((alice mock-player)
    ...
    (setf (tempus::conditions-of alice) (coerce '(0 12 12) 'vector))
    (tempus::interpret-command alice "drink water")
    (char-output-has alice "Your thirst has been quenched.~%Oops, it tasted rather strange!~%")
...
(deftest eat-command ()
  (with-fixtures ((alice mock-player)
                  (bob mock-player)
                  (sandwich mock-object :name "a tasty sandwich"))
    ...
    (char-output-has alice "You eat a tasty sandwich.~%")
    (char-output-has bob "Alice eats a tasty sandwich.~%")
    (destructuring-bind (drunk full thirst)
        (coerce (tempus::conditions-of alice) 'list)
      (is (= drunk 0))
      (is (= full 20))
...
(deftest do-eat-thing/nohunger/no-effect ()
  (with-fixtures ((alice mock-player)
                  (bob mock-player)
                  (sandwich mock-object :name "a tasty sandwich"))
    (tempus::obj-to-char sandwich alice)
    (setf (tempus::kind-of sandwich) tempus::+item-food+)
    (setf (tempus::value-of sandwich) (coerce '(8 0 0 0) 'vector))
    (setf (tempus::conditions-of alice) (coerce '(0 -1 12) 'vector))
    (tempus::interpret-command alice "eat sandwich")
    (char-output-has alice "You eat a tasty sandwich.~%")
    (char-output-has bob "Alice eats a tasty sandwich.~%")
    (destructuring-bind (drunk full thirst)
...
(deftest name-from-drinkcon ()
  (with-fixtures ((canteen mock-object :name "a canteen of water"))
    (tempus::name-from-drinkcon canteen 0)
    (is (null (search "water" (tempus::aliases-of canteen))))))

(deftest name-to-drinkcon ()
  (with-fixtures ((canteen mock-object :name "a canteen"))
...
(deftest pour-into-command ()
  (with-fixtures ((alice mock-player)
                  (bob mock-player)
                  (canteen mock-object :name "a canteen of water")
                  (glass mock-object :name "a glass"))
      ...
      (char-output-has alice "You pour water into a glass.~%")
      (char-output-has bob "Alice pours water into a glass.~%")
...
(deftest pour-into-command-emptying-container ()
  (with-fixtures ((alice mock-player)
                  (bob mock-player)
                  (canteen mock-object :name "a canteen of water")
                  (barrel mock-object :name "a barrel"))
    (setf (tempus::kind-of canteen) tempus::+item-drinkcon+)
    ...
    (char-output-has alice "You pour water into a barrel.~%")
    (char-output-has bob "Alice pours water into a barrel.~%")
...
(deftest pour-out-parser ()
  (with-fixtures ((alice mock-player)
                  (bob mock-player)
                  (canteen mock-object :name "a canteen"))
    (tempus::obj-to-char canteen alice)
...
(deftest pour-out ()
  (with-fixtures ((alice mock-player)
    ...
    (char-output-has alice "You pour water out of a canteen.~%")
    (char-output-has bob "Alice pours water out of a canteen.~%")
    (is (equal '(10 0 0 0) (coerce (tempus::value-of canteen) 'list)))
    (is (null (search "water" (tempus::aliases-of canteen))))))
...
(deftest wield-command/normal/success ()
  (with-fixtures ((alice mock-player)
                  (bob mock-player)
                  (sword mock-object :name "an elvish sword"))
    ...
    (is (equal "You wield an elvish sword.~%"
               (char-output alice)))
    (char-output-has bob "Alice wields an elvish sword.~%")
...
(deftest wield/already-wielding/dual-wield ()
  (with-fixtures ((alice mock-player)
                  (sword-1 mock-object :name "an elvish sword")
                  (sword-2 mock-object :name "an elvish sword"))
    ...
    (setf (tempus::wear-flags-of sword-2) (logior
                                           ...
                                           tempus::+item-wear-wield+))
    (tempus::interpret-command alice "wield sword")
    (char-output-has alice "You wield an elvish sword in your off hand.~%")))

(deftest wield/already-dual-wielding/failure ()
  (with-fixtures ((alice mock-player)
                  (sword-1 mock-object :name "an elvish sword")
                  (sword-2 mock-object :name "an elvish sword")
                  (sword-3 mock-object :name "an elvish sword"))
    (setf (tempus::kind-of sword-1) tempus::+item-weapon+)
    (setf (tempus::kind-of sword-2) tempus::+item-weapon+)
    ...
    (tempus::interpret-command alice "wield sword")
    (char-output-has alice "You don't have a hand free to wield it with.~%")))

(deftest perform-hold/holding-object/success ()
  (with-fixtures ((alice mock-player)
                  (bob mock-player)
                  (wand mock-object :name "a scarred wand"))
    ...
    (is (equal "You grab a scarred wand.~%"
               (char-output alice)))
    (char-output-has bob "Alice grabs a scarred wand.~%")
...
(deftest perform-hold-light/normal/success ()
  (with-fixtures ((alice mock-player)
                  (bob mock-player)
                  (torch mock-object :name "a torch"))
    ...
    (is (equal "You light a torch and hold it.~%"
               (char-output alice)))
    (char-output-has bob "Alice lights a torch and holds it.~%")
...
(deftest perform-attach/normal-scuba/success ()
  (with-fixtures ((alice mock-player)
                  (bob mock-player)
                  (mask mock-object :name "a scuba mask")
                  (tank mock-object :name "a scuba tank"))
...
(deftest perform-attach/normal-bomb/success ()
  (with-fixtures ((alice mock-player)
                  (bob mock-player)
                  (fuse mock-object :name "a fuse")
                  (bomb mock-object :name "a bomb"))
...
(deftest perform-conceal/normal/success ()
  (with-fixtures ((alice mock-player)
    ...
    (setf (tempus::wear-flags-of wand) (logior
                                        tempus::+item-wear-take+
                                        tempus::+item-wear-hold+))
    (tempus::perform-conceal alice wand)
    (is (equal "You conceal a scarred wand.~%"
               (char-output alice)))
    (char-output-has bob "Alice conceals something.~%")))
...
(deftest perform-sacrifice/normal/success ()
  (with-fixtures ((alice mock-player)
    ...
    (is (equal "You sacrifice a scarred wand.~%"
               (char-output alice)))
    (char-output-has bob "Alice sacrifices a scarred wand.~%")
    (is (null (tempus::carrying-of alice)))))
...
(deftest donate/normal/success ()
  (with-fixtures ((alice mock-player)
      ...
      (is (equal "You donate a scarred wand.~%"
                 (char-output alice)))
      (char-output-has bob "Alice donates a scarred wand.~%")
      (is (null (tempus::carrying-of alice)))
