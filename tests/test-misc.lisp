...
(deftest do-aliases/no-args-with-aliases/displays-all-aliases ()
  (with-fixtures ((alice mock-player))
    (setf (tempus::command-aliases-of alice) '(("foo" "bar") ("baz" "quux")))
    (tempus::interpret-command alice "aliases")
    (char-output-is alice "Currently defined aliases:~% &cfoo             &nbar~% &cbaz             &nquux~%")))

(deftest do-aliases/no-args-no-aliases/displays-none ()
  (with-fixtures ((alice mock-player))
    (tempus::interpret-command alice "aliases")
    (char-output-is alice "Currently defined aliases:~% None.~%")))

(deftest do-aliases/one-args-matching-aliases/displays-matching ()
  (with-fixtures ((alice mock-player))
    (setf (tempus::command-aliases-of alice) '(("foo" "bar") ("baz" "quux")))
    (tempus::interpret-command alice "aliases foo")
    (char-output-is alice "Aliases matching 'foo':~% &cfoo             &nbar~%")))

(deftest do-aliases/one-args-no-matching-aliases/displays-none ()
  (with-fixtures ((alice mock-player))
    (setf (tempus::command-aliases-of alice) '(("foo" "bar") ("baz" "quux")))
    (tempus::interpret-command alice "aliases bak")
    (char-output-is alice "Aliases matching 'bak':~% None.~%")))

(deftest do-alias/new-alias/adds-new-alias ()
  (with-fixtures ((alice mock-player))
    (tempus::interpret-command alice "aliases bak boo")
    (is (equal '(("bak" "boo")) (tempus::command-aliases-of alice)))
    (char-output-is alice "Alias 'bak' added.~%")))

(deftest do-alias/existing-alias/modifies-alias ()
  (with-fixtures ((alice mock-player))
    (setf (tempus::command-aliases-of alice) '(("foo" "bar") ("baz" "quux")))
    (tempus::interpret-command alice "aliases foo bak")
