...
(deftest do-oset-aliases/sets-object-aliases ()
  (with-fixtures ((alice mock-player :level 51 :override-security t)
                  (armor mock-object :name "some plate armor"))
    (tempus::obj-to-char armor alice)
    (tempus::interpret-command alice "oset armor alias shiny sword")
    (char-output-is alice "Object aliases set to 'shiny sword'.~%")
    (is (equal "shiny sword" (tempus::aliases-of armor)))))

(deftest do-oset-name/sets-object-name ()
  (with-fixtures ((alice mock-player :level 51 :override-security t)
                  (armor mock-object :name "some plate armor"))
    (tempus::obj-to-char armor alice)
    (tempus::interpret-command alice "oset armor name a shiny sword")
    (char-output-is alice "Object name set to 'a shiny sword'.~%")
    (is (equal "a shiny sword" (tempus::name-of armor)))))

(deftest do-oset-material/sets-object-material ()
  (with-fixtures ((alice mock-player :level 51 :override-security t)
                  (armor mock-object :name "some plate armor"))
    (tempus::obj-to-char armor alice)
    (tempus::interpret-command alice "oset armor material water")
    (char-output-is alice "Object material set to water.~%")
    (is (= 1 (tempus::material-of armor)))))

(deftest do-oset-extra2/sets-object-extra2-flags ()
  (with-fixtures ((alice mock-player :level 51 :override-security t)
                  (armor mock-object :name "some plate armor"))
    (tempus::obj-to-char armor alice)
    (tempus::interpret-command alice "oset armor extra2 + ablaze noremove")
    ...
    (is (= (logior tempus::+item2-ablaze+ tempus::+item2-noremove+)
           (tempus::extra2-flags-of armor)))))

(deftest do-oset-apply/sets-object-apply ()
  (with-fixtures ((alice mock-player :level 51 :override-security t)
                  (armor mock-object :name "some plate armor"))
    (tempus::obj-to-char armor alice)
    (tempus::interpret-command alice "oset armor apply str 5")
    (char-output-is alice "Done.~%")
    (is (= tempus::+apply-str+ (tempus::location-of (aref (tempus::affected-of armor) 0))))
    (is (= 5 (tempus::modifier-of (aref (tempus::affected-of armor) 0))))))
